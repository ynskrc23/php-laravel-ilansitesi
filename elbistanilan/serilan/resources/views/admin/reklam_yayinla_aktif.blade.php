...
<div style="margin-top:15px; margin-left:10px;" class="container-fluid">
  <div class="row content">
    ...
    <div class="col-sm-9">
		...
		<div style="margin-top:16px; background-color:#607d8bb5; color:white; text-align:center;" class="well well-sm">Aktif Kayıtlar</div>   
		...
		<table class="table table-hover">
          ...
          <a href="/reklam-yayinla-pasif" style="margin-left:20px; background:#ddd;" class="btn btn-default">Pasif Reklam </a>
          <tr>
      ...
      <tbody>
          @foreach ($all_reklam_aktif as $ra) 
              <tr class="odd gradeX">
                
                <td>
                    @if($ra->reklamy_image != null)
                    <img src="{{\Illuminate\Support\Facades\Storage::url($ra->reklamy_image)}}" style="height: 80px" width="160px;"> </td>
                
                    @else                                       
                    <img src="{{'../images/home/resimyok.png'}}" style="height: 80px" width="160px;"> </td>
                
                     @endif
                     <td> {{$ra->reklamy_alan}} </td>
                   <td> {{Carbon::parse($ra->created_at)->diffForHumans()}}  </td>
                <td class="center">
                    @if($ra->reklamy_durum==1)
                ...
                <td class="center">  
                <a href="/edit-reklamyayinla/{{$ra->reklamy_id}}" class="btn btn-info"> Güncelle </a>                                                                                                     
                <a href="/delete-reklamyayinla/{{$ra->reklamy_id}}" class="btn btn-danger"  onclick="return ShowConfirm();"> Delete </a>                                            
                </td>   
              </tr> 
          @endforeach
      </tbody>
		...
		<div class="text-center"> {{ $all_reklam_aktif->links() }} </div>
		</div>
    </div>
